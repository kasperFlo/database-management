...
#check what the user wants to check/read  : IE certain dicts or certain values of dicts
    ...
    elif (len(userImp)==1 and ((len(currentDB))-((ord(userImp.upper()))-65) in range(1,len(currentDB)+1))) : # A,B,C,D,E,F
        lookingKey = False ; userImp = ord(userImp.upper())-65
...
def update():
    ...
    printData('','v2')
    space = input('What position would you like to spot to change (IE: A15 / 15A)> ') 
